...
The remaining evidence supports exactly that reading. Across six real-world domains
...
purpose-built diversity and routing baselines on spatial coverage with none of their
machinery. Four further experiments close the design space: the router's forgetting
reappears with \emph{gradient-trained neural experts} on two standard
continual-learning benchmarks (permuted-digits $+62\%$, Split-CIFAR-100 $+22\%$);
a \emph{hybrid} reservation router isolates reward-independence of \emph{protection}
as the operative property; and intra-regime-drift and population-sizing sweeps map
the mechanism's own failure modes. We close with an evidence-tiered survey of
applications---regime-switching finance and edge sensing, elastic memory for
agentic-AI and mixture-of-experts systems, and further candidates. Code and data:
