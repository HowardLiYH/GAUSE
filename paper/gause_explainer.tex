...
forgets dormant regimes, while every reward-independent assignment retains the
regimes it covers---and competition is the cheapest arm to secure both
reward-independence and guaranteed coverage at once ($+71\%$ lower
post-reactivation error versus the reward-driven router, robust to swapping hard
eviction for soft interference). We are careful about scope and rigor: the
comparison is \emph{task-incremental} (every arm, GAUSE included, sees the regime
label, so it is information-symmetric); we benchmark against the honest ceiling---an
\emph{oracle fixed assignment} given both the labels and a perfect covering
permutation---which GAUSE recovers without being handed the assignment (within
$0.030$ at $K{=}1$, indistinguishable for $K\ge3$). Crucially,
the central retention result survives \emph{dropping the label}: a class-incremental
variant that infers the active regime from competence alone self-organizes against
the hidden regimes and retains ($0.38$ post-reactivation vs.\ $1.07$ for a
label-free forgetting monolith), detecting reactivation from input similarity with
no regime identity supplied. Empirical results across six real-world
...
$=0.65$ from competition alone) and matches purpose-built diversity and routing
baselines on spatial coverage with none of their machinery. Four further
experiments close the design space: the router's forgetting reappears with
\emph{gradient-trained neural experts} on a standard continual-learning benchmark
($+62\%$ lower post-reactivation test error for GAUSE, confirming the failure is
architecture-agnostic); a \emph{hybrid} router with an explicit reservation term
recovers retention only when spare capacity exists, isolating
reward-independence of \emph{protection} as the operative property; an
\emph{intra-regime drift} experiment quantifies the converse risk---stale
specialists---and a lightweight staleness trigger that recovers most of it; and a
population-sizing sweep shows surplus agents idle harmlessly while scarce agents
under-cover. We close with an evidence-tiered survey of application
domains---regime-switching finance and edge sensing (validated data families),
elastic memory for agentic AI and mixture-of-experts systems (mechanism-level
evidence), and further candidates---where the mechanism's parsimony and retention
guarantee are most valuable. Code and data:
...
\emph{gradient-trained neural experts} on a standard continual-learning benchmark
(the failure is architecture-agnostic), and stress-test it with a \emph{hybrid}
reward-driven router carrying an explicit reservation term, which retains exactly
when its protection becomes reward-independent. The GAUSE algorithm is then one
...
detecting reactivation purely from input similarity---turning the hardest open
question from a promissory note into a demonstrated, if still imperfect, result.
...
prediction six-for-six (Table~\ref{tab:fivearm}), including the hybrid router, which
sits \emph{outside} the class and partially retains exactly when its protection
becomes reward-independent. The principle is best read as an organizing observation
...
=\Pr[U_T<K]\le (1-p)^{\,T-K+1}\!\cdot\binom{T}{K-1}\to 0$ as $T\to\infty$; under the
soft model the retained strength decays as $(1-\rho)^{U_T}\to 0$ with $\rho$ the
per-update interference rate. In both cases $\Pr[\text{reallocation}]\to 1$.
...
continual-learning experiment (Section~\ref{sec:closing}), which is the one
piece of evidence here that goes beyond analogy. \textbf{Multi-robot fleets}
...
architecture-agnostic: with gradient-trained MLP experts on permuted-digits, the
reward-driven router forgets while GAUSE retains ($+62\%$ lower post-reactivation error
at $E{=}R$), converting the central prediction---and the agentic-AI/MoE application
...
similarity. What remains open is scaling label-free recovery to many overlapping
regimes and to real, feature-rich observations, and closing the modest
inference gap to the label-given ceiling ($0.38$ vs.\ $0.283$); the correspondence
between emergent win-clusters and a stable latent labeling (online
competitive-clustering / EM) is the theoretical core of that program.
