...
\begin{intuition}[Why EG]
Equation~\eqref{eq:eg_affinity_update} is mirror descent on the simplex
...
\paragraph{Historical note (V3 \(\to\) V4).}
Earlier versions of this work~\cite{li2026emergent} used the additive
heuristic
\begin{equation*}
\alpha_{i^*,r}^{(t+1)} =
\begin{cases}
\alpha_{i^*,r}^{(t)} + \eta(1 - \alpha_{i^*,r}^{(t)}) & r = r_t \\
\alpha_{i^*,r}^{(t)} - \dfrac{\eta}{R-1} & r \neq r_t
\end{cases}
\end{equation*}
followed by clamping and post-hoc normalization. That heuristic was
not derived from a regularization principle, and required an undocumented
floor $\max(0.01,\,\cdot)$ to prevent negative entries in steady state.
The EG update in Eq.~\eqref{eq:eg_affinity_update} resolves all three
issues simultaneously without changing the qualitative dynamics.
% TODO[V4]: Replace this paragraph with a cross-reference to the
% V3-vs-V4 empirical comparison section once Batch~C is complete.
...
Under the exponentiated-gradient affinity update (Eq.~\ref{eq:eg_affinity_update})
with learning rate $\eta$, the geometric-convergence bound holds in the
log-ratio coordinate. Letting $\ell_{i,r}^{(t)} = \log \alpha_{i,r}^{(t)}$
and $\ell_{i,r}^{*} = \log \alpha_{i,r}^{*}$, the gap satisfies
\begin{equation}
\bigl|\ell_{i,r}^{(t)} - \ell_{i,r}^{*}\bigr|
\;\leq\;
\bigl|\ell_{i,r}^{(0)} - \ell_{i,r}^{*}\bigr| + \eta \cdot |W_i(r,t) - W_i(r_t \neq r, t)|
\end{equation}
where $W_i(r,t)$ is the number of wins for agent $i$ in regime $r$ up
to time $t$. This is the standard multiplicative-weights regret bound
specialized to the indicator-reward setting~\cite{arora2012multiplicative}.
\end{proposition}

% TODO[V4]: Formalize and re-prove the convergence statement above for
% the EG update. The current statement is a placeholder reformulation
% of the V3 additive-update bound. A complete proof using mirror
% descent regret bounds is deferred to follow-up work; see
% docs/V4_EG_RENOVATION_AUDIT.md Section 6.3.
