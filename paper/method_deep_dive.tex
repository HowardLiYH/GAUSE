...
\midrule
Regularization (EWC) & A ``don't-touch'' lock on important variables & (not tested---needs per-parameter importance) \\[4pt]
Replay / rehearsal & Keeping a log and re-running old jobs & (not tested---needs stored data) \\[4pt]
...
\[
  ...
  \;\le\; \binom{T}{K-1}(1-p)^{\,T-K+1} \;\xrightarrow[T\to\infty]{}\; 0,
\]
and $\Pr[\text{reallocation}]\to 1$ (under the soft model the retained strength is
$\le s_0\,\kappa^{\,U_T}\to0$). For a reward-independent allocator $A_t[r]$ does not
