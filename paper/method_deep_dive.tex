...
\begin{keypoint}
\textbf{Honest synthesis.} NichePopulation reliably produces coordination-free
...
Across all five arms the retention results align on a single axis:
\emph{assignments independent of current task reward} (fixed random niches,
intrinsic-reward diversity, converged competitive exclusion) retain dormant
regimes, while allocations that chase current reward (monolith, learned router)
forget them. Coverage is a commodity; reward-independent assignment is not, and
competition is its cheapest emergent source.
...
\begin{table}[h]
\centering
\caption{SI Comparison: NichePopulation vs. MARL Baselines (10 trials per domain)}
...
\begin{keypoint}
NichePopulation reaches effectively maximal specialization (SI $=1.000$), while
...
\textbf{NichePopulation specialist SI per domain} (the rare-regime probe stores
...
\begin{itemize}
    ...
    rare regimes (weather, commodities, crypto, traffic) NichePopulation beats
    every MARL method by $+11.4\%$ to $+50.1\%$.
    \item \textbf{No advantage where structure is absent}: in air quality and
    solar---the low oracle-gap domains---NichePopulation is statistically tied
...
def run_niche_population(regimes, regime_probs, n_agents, n_iterations,
                          niche_bonus, seed):
    """Run NichePopulation simulation (V4 EG update)."""
...
\textbf{A:} Yes. All 6 domains use real data, and under the canonical V4 (EG)
update at $\lambda=0.3$ every domain reaches SI $>0.98$, with Cohen's
$d>50$ and $p<10^{-80}$ throughout.
