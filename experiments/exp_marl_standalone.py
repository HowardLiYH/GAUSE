...
.. note::
   **Secondary MARL run; not the source for the paper's MARL figure.** This
   standalone harness uses a simplified environment/SI measurement and yields a
   more conservative NichePopulation SI (~0.76, MARL ~0.17); it writes
   ``results/marl_comparison/latest_results.json``. The MARL figure in
   ``paper/main.tex`` (NichePopulation SI = 1.000, every MARL baseline < 0.02,
   5,000 training episodes) is generated by ``exp_marl_comparison.py`` ->
   ``results/real_marl_comparison/results.json``. The two differ by SI
   methodology/training setup; cite ``real_marl_comparison`` for the paper and
   treat this file as a robustness cross-check (the qualitative gap---
   NichePopulation specializes, MARL homogenizes---holds in both).
"""
